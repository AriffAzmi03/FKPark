...
if (isset($_GET['summonID'])) {
    ...
    $query = "SELECT s.summonID, s.vehiclePlateNum, s.summonViolationType, s.summonDemerit, s.summonDate, v.studentID, v.vehicleType, v.vehicleBrand, v.vehicleColour, v.studentID, st.studentName, st.studentPhoneNum, st.studentAddress, st.studentType, st.studentYear, st.studentEmail
              FROM summon s
              JOIN vehicle v ON s.vehiclePlateNum = v.vehiclePlateNum
              JOIN student st ON v.studentID = st.studentID
              WHERE s.summonID = ? AND st.studentID = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $summonID, $_SESSION['studentID']);
    $stmt->execute();
    ...
    if (!$summon) {
        echo "<div class='alert alert-danger' role='alert'>Invalid request or you do not have permission to view this summon.</div>";
        exit();
    }
} else {
...
<div class="container mt-4">
    <ol class="breadcrumb">
        <li class="breadcrumb-item">
            <a href="student-manage-summon.php">Summons</a>
        </li>
